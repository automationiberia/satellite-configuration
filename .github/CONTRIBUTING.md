...
Ensure you install the `redhat.satellite` collection, so that roles and playbooks can be properly linted:
`ansible-galaxy collection install redhat.satellite -p collections/`

Python's pre-commit tool can be installed, and hooks installed, to cleanup whitespace, newlines, and run yamllint and ansible-lint against your local changes before committing. This will help you avoid failures in the github workflows. To install them, run the following commands:

1. Create a local virtual environment for `satellite_configuration` (suggested, its your system!): `virtualenv ~/satellite_configuration_venv && source ~/satellite_configuration_venv/bin/activate`
...
4. If you are creating a new role, please add a test for it in our [testing directory.](https://github.com/automationiberia/satellite_configuration/blob/devel/tests/) by adding a new role entry and adding the appropriate yaml file with test data in the `satellite_output` directory.
